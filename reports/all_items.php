...
function generateAdminRow()
{
   ...
   include_once('../db.php');
    //for admin only
    $sql = "SELECT * FROM stock,department,supplier WHERE item.dept_id = department.dept_id AND item.supplier_id = supplier.supplier_id";
        //use for MySQLi OOP
...
function generateDeptRow()
{
    ...
    include_once('../db.php');
    //for admin only
    $sql = "SELECT * FROM stock WHERE `dept_id` = '". $_SESSION['dept_id'] ."'";
    //use for MySQLi OOP
    $query = $conn->query($sql);
    $i = 1;
    while ($row = $query->fetch_assoc()) {
